...
conn = mysql.connector.connect(
    host='localhost',
    user='root',
    password='harkiratpass',
    port = 3306,
    database='harkiratproject'
)
cursor = conn.cursor()
...
# Similarly Create notes table if not exists
cursor.execute("""
...
class PHRServer(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/notes':
            ...
            self._set_response()
            response = {
                'message': 'Welcome to Personal Health Record system',
                'endpoints': [
                    {'login': 'Login endpoint'},
                    {'signup': 'Signup endpoint'},
                    {'notes': 'Get your health notes'}
                ]
            }
